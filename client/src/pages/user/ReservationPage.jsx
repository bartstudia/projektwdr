...
const ReservationPage = () => {
  ...
  useEffect(() => {
    if (!selectedDate || !lakeId) return;
    fetchReservedSpots();
  }, [selectedDate, lakeId]);

  const fetchReservedSpots = async () => {
    if (isSpotUnavailable) {
      const message = 'To stanowisko jest juz zarezerwowane w wybranym terminie.';
      setError(message);
      alert(message);
      return;
    }
    ...
    try {
      ...
      const dateStr = selectedDate instanceof Date
        ...
        : selectedDate;
      const data = await reservationService.getReservedSpotsForDate(lakeId, dateStr);
      setReservedSpotIds(data.reservedSpotIds || []);
    } catch (err) {
      setAvailabilityError(err.message || 'Blad podczas sprawdzania dostepnosci');
      setReservedSpotIds([]);
    } finally {
  ...
  const isSpotUnavailable = selectedDate && reservedSpotIds.includes(spotId);
